...
function Get-NutanixVirtIODrivers {
    Param(
        [parameter(Mandatory=$true)]
        [int]$BuildNumber,
        [parameter(Mandatory=$true)]
    ...
    Write-Log "Getting Nutanix Virtual IO Drivers: $BasePath..."
    $osVersion = $null
    ...
    if (!$osVersion -and $RecursionDepth -lt 1) {
        # Fallback to 2012r2/w8.1 if no drivers are found
        return (Get-NutanixVirtIODrivers -BuildNumber 9600 -IsServer $IsServer `
            -BasePath $BasePath -Architecture $Architecture -RecursionDepth 1)
    }
    ...
    Write-Log "Finished to get Nutanix Virtual IO Drivers."
    return $driverPaths
}
...
function Add-VirtIODrivers {
    ...
    if (!$isNutanixTarget) {
        ...
        $virtioDriversPaths = Get-NutanixVirtIODrivers -BuildNumber $buildNumber `
            -IsServer ([bool](Is-ServerInstallationType $image)) -BasePath $driversBasePath `
            -Architecture $image.ImageArchitecture
    }
